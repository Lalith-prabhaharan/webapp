...
const createDiscussion = async (req, res) => {
  try {
    const { courseId, userId, topic, description } = req.body;
    const discussion = new Discussion({ courseId, userId, topic, description });
...
const getDiscussionsByCourse = async (req, res) => {
    try {
      ...
      const discussions = await Discussion.find({ courseId }).populate('userId', 'name');
